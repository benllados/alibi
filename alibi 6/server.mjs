...
export const game = new Game({aiDecoys,aiSketches,leadMs:1800,onChange(room){for(const c of connections.get(room.code)||[]){try{c.res.write(`data: ${JSON.stringify(game.snapshot(room,c.token))}\n\n`);}catch{c.res.end();}}}});
...
const files=new Map([['/','index.html'],['/join','index.html'],['/host','index.html'],['/app.js','app.js'],['/style.css','style.css'],['/favicon.svg','favicon.svg'],['/favicon-yellow.png','favicon-yellow.png'],['/favicon-smiley.webp','favicon-smiley.webp'],...['gameplay.css','gameplay-cast.js','gameplay-ui.js','qr.js','entrance.css','playground.css','playground.js','playground-scene.js','media/alibi-preview.mp4','media/preview-poster.webp','media/alibi-preview.vtt','music/paper-trails-bed.mp3','music/paper-trails-spark.mp3','music/theme.json','characters.js','art.js','audio.js','logo.js','fonts/smile-moon.otf','art/paper.svg'].map(file=>['/'+file,file])]);
function updatePresence(room){
 const live=connections.get(room.code)||new Set();room.presence={};for(const p of room.players)room.presence[p.id]=[...live].some(c=>c.token===p.token);game.changed(room);
}
