...
test('Thirty-second writing and voting deadlines progress on expiry',()=>{const g=setup();assert.equal(g.r.deadline-g.now(),30000);g.act(g.seat(g.r.t.subject),'answer',{text:'A sandwich'});g.advance(29999);assert.equal(g.r.phase,'write');g.advance(1);assert.equal(g.r.phase,'vote');assert.equal(g.r.deadline-g.now(),30000);g.advance(29999);assert.equal(g.r.phase,'vote');g.advance(1);assert.equal(g.r.phase,'reveal');});

test('Question writing is capped at sixty seconds, with on-topic fallback metadata',()=>{const g=setup();for(let i=0;i<4;i++){g.act(g.host,'advance');g.act(g.host,'next');}assert.equal(g.r.phase,'questions');assert.equal(g.r.deadline-g.now(),60000);g.advance(59999);assert.equal(g.r.phase,'questions');g.advance(1);assert.equal(g.r.phase,'write');assert.equal(Object.keys(g.r.questions).length,4);assert.ok(g.r.t.decoys.length>=4);assert.equal(g.r.t.prompt,g.r.questions[g.r.t.subject].text);assert.deepEqual(g.r.t.decoys,g.r.questions[g.r.t.subject].decoys);});

for(const n of [3,4,8])test(`${n} players: subject watches, one OTHER artist knows, and drawing lasts ninety seconds`,()=>{
 const g=setup(n);finale(g);assert.equal(g.r.deadline-g.now(),30000);const subject=g.r.t.subject,artist=g.r.t.artist;assert.notEqual(artist,subject);
